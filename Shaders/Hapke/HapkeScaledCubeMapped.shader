Shader "Custom/HapkeScaledCubeMapped"
{
    Properties
    {
        ...
        [Space(10)]
        [Header(Planet Cubemaps)]
        [Space(10)]
        _ColorCube("Planet Color Cubemap", CUBE) = "" {}
        _BumpCube("Planet Bump Cubemap", CUBE) = "bump" {}
        _HeightCube("Planet Height Cubemap", CUBE) = "black" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Interpolators Domain_Shader(TessellationFactors factors, OutputPatch<TessellationControlPoint, 3> patch, float3 barycentricCoordinates : SV_DomainLocation)
            {
                ...
                // Sample height cubemap — single hardware lookup replaces 6-texture face selection
                float3 heightDir = normalize(o.biplanarTextureCoords);
                // heightDir.x = -heightDir.x;
                // heightDir.z = -heightDir.z;
                float displacement = texCUBElod(_HeightCube, float4(heightDir, 0)).r;
            ...
            {   
                ...
                float3 cubeDir = normalize(localPos);
                // cubeDir.x = -cubeDir.x;
                // cubeDir.z = -cubeDir.z;
                ...
                // Height value — single cubemap sample
                float planetHeight = texCUBE(_HeightCube, cubeDir).r;
                ...
                // Planet normal from cubemap — single cubemap sample
                float3 planetNormal = UnpackScaleNormal(texCUBE(_BumpCube, cubeDir), _PlanetBumpScale);
                // planetNormal.x = -planetNormal.x;  // compensate for X mirror
                ...
                // Color cubemap — single hardware sample
                float4 diffuseColor = texCUBE(_ColorCube, cubeDir);
        ...
        //         // Sample height cubemap — single hardware lookup
        //         float3 heightDir = normalize(v.biplanarTextureCoords);
        //         heightDir.x = -heightDir.x;
        //         heightDir.z = -heightDir.z;
        //         float displacement = texCUBElod(_HeightCube, float4(heightDir, 0)).r;
