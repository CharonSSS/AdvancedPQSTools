...
namespace AdvancedPQSTools
{
    ...
    public class AdvancedPQSToolsEditor : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (_cams == null)
            {
                ...
                foreach (Camera cam in cameras)
                {
                    ...
                    {
                        Debug.Log($"[AdvancedPQSTools] Exception getting camera {cam.name}\n{exceptionStack}");
                    }
                }
            }
        ...
        public void OnGUI()
        {
            if (_GUIOpen)
            {
                if (HighLogic.LoadedSceneIsFlight && FlightGlobals.ActiveVessel != null)
                    windowPosition = GUILayout.Window(69105, windowPosition, ShowGUI, "AdvancedPQSTools Parameters", windowStyle);
            }
        }
        ...
        private void ShowGUI(int windowID)
        {
            ...
            GUILayout.Label("RunwayFix");

            GUILayout.BeginHorizontal();
            GUILayout.Label("collidersDisabled: ");
            GUILayout.Label(RunwayFix.Instance.collidersDisabled.ToString(), GUILayout.ExpandWidth(false));
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            GUILayout.Label("isOnRunway: ");
            GUILayout.Label(RunwayFix.Instance.isOnRunway.ToString(), GUILayout.ExpandWidth(false));
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            GUILayout.Label("hold: ");
            GUILayout.Label(RunwayFix.Instance.hold.ToString(), GUILayout.ExpandWidth(false));
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            GUILayout.Label("lastHitCollider: ");
            GUILayout.Label(RunwayFix.Instance.lastHitColliderName, GUILayout.ExpandWidth(false));
            GUILayout.EndHorizontal();
