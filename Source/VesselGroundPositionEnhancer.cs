        ...
        private void OnVesselGoOffRails(Vessel v)
        {
            if (v.Landed && !v.Splashed)
            {
                if (v.situation == Situations.PRELAUNCH)
                {
                    ...
                    Debug.Log($"[AdvancedPQSTools-VGPE] Vessel going off rails in PRELAUNCH, skipping {framesToSkip} frames on CollisionEnhancer.");
                }
                else
                {
                    ...
                    Debug.Log($"[AdvancedPQSTools-VGPE] CheckGroundCollision() in {sw.ElapsedMilliseconds}ms");
                    sw.Stop();
                }
            }
            else
            {
                Debug.Log("[AdvancedPQSTools-VGPE] not landed");
            }
        }
    }
}
