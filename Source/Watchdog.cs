...
namespace AdvancedPQSTools
{
    ...
    public class WatchDogs : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            foreach (Camera cam in cameras)
            {
                ...
                if (nearClip > 0)
                {
                    ...
                    Debug.Log($"[AdvancedPQSTools] Watchdog: Setting camera {cam.name} near clip to {nearClip} so camera now has {cam.nearClipPlane}");
                }

                if (farClip > 0)
                {
                    cam.farClipPlane = farClip;
                    Debug.Log($"[AdvancedPQSTools] Watchdog: Setting camera {cam.name} far clip to {farClip} so camera now has {cam.farClipPlane}");
                }
            }
        }
        ...
        private void OnVesselSituationChanged(GameEvents.HostedFromToAction<Vessel, Vessel.Situations> data)
        {
            ...
            {
                isSuborbital = false;
                Debug.Log("[AdvancedPQSTools] Calling StartUpSphere() to prevent missing PQ tiles");
